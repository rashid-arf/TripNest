    ...
    <div className={`content-width ${styles.TravelGuideContainer}`}>
      ...
      >
        {travelGuide.map((item) => (
          <SwiperSlide key={item.id} className={styles.TravelGuideSlideItem}>
            <Image src={item.image} alt={item.alt} width={200} height={200} />
            <div className={styles.TravelGuideSlideItemDescription}>
              <p>{item.guideName}</p>
              <p className={styles.TravelGuideSlideItemDescriptionPosition}>{item.guidePosition}</p>
            </div>
          </SwiperSlide>
        ))}
      </Swiper>
